...
// apply dns settings from config.xml to the actual device resolv.conf
void applydnstodevice(){
  FILE *fp = fopen("config.xml", "r");
  if(!fp){ perror("config.xml"); return; }
  ...
  while(fgets(line, sizeof(line), fp)){
    ...
    if((s = strstr(line, "<searchdomain>")) && (e = strstr(line, "</searchdomain>"))){
      int len = e - (s + 14);
      if(len > 0 && len < (int)sizeof(searchdomain)){
  ...
  if(!addr[0]){ printf("[DNS] no dns address found in config.xml\n"); return; }

  FILE *resolv = fopen("/etc/resolv.conf", "w");
  if(!resolv){ perror("/etc/resolv.conf"); return; }

  fprintf(resolv, "# generated by fakecamera SetDNS\n");
  if(searchdomain[0]) fprintf(resolv, "search %s\n", searchdomain);
  fprintf(resolv, "nameserver %s\n", addr);
  fflush(resolv);
  fsync(fileno(resolv));
  fclose(resolv);
  printf("[DNS] applied nameserver=%s search=%s to /etc/resolv.conf\n", addr, searchdomain);
}
